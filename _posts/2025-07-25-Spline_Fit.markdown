...
- $$\text{rssXY} = \frac{1}{2}(\text{CRLB}_x + \text{CRLB}_y)$$ (en **pixels**) : erreur quadratique moyenne sur la localisation latérale.
- $$\text{rssZ} = \text{CRLB}_z$$ (en **voxels**). Elle peut être convertie en nanomètres par $$\text{rssZ}_{\text{nm}} = \Delta_z \cdot \text{CRLB}_z$$ si besoin.
...
Chaque point ainsi traité produit une localisation enrichie : $$[x, y, z, \text{intensité}, \text{offset}, \text{rssXY}, \text{rssZ}]$$.
...
Pour la 3D, il suffit de rajouter encore uen dimension de coefficients : 
